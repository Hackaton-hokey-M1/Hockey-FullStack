...
export default function MatchHeader({ match: initialMatch }: { match: Match }) {
  const [match, setMatch] = useState<Match>(initialMatch);
  const [isUpdating, setIsUpdating] = useState(false);
  const { applyUpdateToMatch, liveUpdates } = useMatchesLive();

  // Appliquer les mises à jour SSE
  useEffect(() => {
  ...
  // Mettre à jour si le match initial change
  useEffect(() => {
    setMatch(initialMatch);
  }, [initialMatch]);

  // Détecter les changements pour l'animation
  useEffect(() => {
    setIsUpdating(true);
    ...
    >
      ...
      {isUpdating && (
        <motion.div
          ...
          className="absolute inset-0 bg-linear-to-r from-blue-500/10 to-cyan-500/10 pointer-events-none"
        />
      )}
