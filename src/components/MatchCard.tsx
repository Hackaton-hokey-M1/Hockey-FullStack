...
export default function MatchCard({ match }: MatchCardProps) {
  ...
  const href = `/${locale}/matches/${match.id}`;
  const [isUpdating, setIsUpdating] = useState(false);

  // Détecter les changements de score ou statut pour afficher une animation
  useEffect(() => {
    setIsUpdating(true);
  ...
  return (
    <Link
      href={href}
      aria-label={`Voir le match ${match.homeTeam.name} - ${match.awayTeam.name}`}
      className="block"
    >
      <motion.div
        ...
        >
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex-1 flex items-center justify-between gap-4">
                ...
                <div className="flex flex-col items-center justify-center gap-2 px-4">
                  ...
                  {/* Status Badge */}
                  <motion.span
                    key={`status-${match.id}-${match.status}`}
          ...
          {isUpdating && (
            <motion.div
              ...
              transition={{ duration: 0.8 }}
              className="absolute inset-0 rounded-3xl border-2 border-blue-500 pointer-events-none"
            />
          )}
        </motion.div>
      </motion.div>
